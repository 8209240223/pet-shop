...
<main class="layout"> <!-- 定义 JSP 页面内容容器。 -->
    <section class="workspace"> <!-- 定义 JSP 管理区域。 -->
        <div class="section-title"> <!-- 定义标题行。 -->
            <h1>${title}</h1> <!-- 输出管理页面标题。 -->
            <a href="/">返回首页</a> <!-- 返回现代前端首页。 -->
        </div> <!-- 结束标题行。 -->
        <div class="list-box"> <!-- 定义低库存列表容器。 -->
            <c:forEach items="${products}" var="product"> <!-- 使用 JSTL 遍历低库存商品。 -->
                <div class="line-item"><span>${product.name} / ${product.category}</span><strong>库存 ${product.stock}</strong></div> <!-- 输出低库存商品行。 -->
            </c:forEach> <!-- 结束低库存商品遍历。 -->
        </div> <!-- 结束低库存列表容器。 -->
    </section> <!-- 结束 JSP 管理区域。 -->
</main> <!-- 结束 JSP 页面内容容器。 -->
