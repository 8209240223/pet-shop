...
<main class="layout"> <!-- 定义 JSP 页面内容容器。 -->
    <section class="workspace"> <!-- 定义 JSP 技术矩阵区域。 -->
        <div class="section-title"> <!-- 定义标题行。 -->
            <h1>${title}</h1> <!-- 输出技术矩阵标题。 -->
            <a href="/">返回首页</a> <!-- 返回现代前端首页。 -->
        </div> <!-- 结束标题行。 -->
        <div class="list-box"> <!-- 定义矩阵列表容器。 -->
            <c:forEach items="${matrix}" var="row"> <!-- 使用 JSTL 遍历矩阵行。 -->
                <div class="line-item"><span>${row.technology}：${row.purpose}</span><strong>5 个入口</strong></div> <!-- 输出技术矩阵行。 -->
            </c:forEach> <!-- 结束矩阵遍历。 -->
        </div> <!-- 结束矩阵列表容器。 -->
    </section> <!-- 结束 JSP 技术矩阵区域。 -->
</main> <!-- 结束 JSP 页面内容容器。 -->
</body> <!-- 结束 JSP 页面主体。 -->
</html> <!-- 结束 JSP 页面。 -->
